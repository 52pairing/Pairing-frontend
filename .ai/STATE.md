...
## 현재 작업 (2026-08-19 — 비로그인 헤더 회사소개 메뉴 제거)

- 작업명: 비로그인 메인 헤더의 `회사 소개` 메뉴 제거
- 관련 Issue: 생성 전
- 관련 브랜치: 현재 작업 브랜치
- 진행 상황: 완료
- 작업 범위: `GuestHeader`의 `/about` 링크 제거, 비로그인 헤더 메뉴 회귀 테스트 추가
- 확인 결과: `src/app/about/page.tsx` 등 `/about` 페이지는 현재 저장소에 존재하지 않아 삭제할 관련 페이지 파일 없음
- API 변경: 없음
- 검증: 관련 Jest 1 suite/1 test, 변경 파일 ESLint, 프로덕션 빌드, `git diff --check` 통과
- 실제 브라우저: 미실행 — 코드·테스트·빌드로 메뉴 제거와 나머지 링크 유지를 확인

## 현재 작업 (2026-08-18 — 협상 타결 후 계약관리 이동 동선 추가)

- 작업명: 협상 완료 화면에 `계약관리로 가기` 버튼 추가
- 관련 Issue: 생성 전
- 관련 브랜치: 현재 작업 브랜치
- 진행 상황: 완료
...
# 현재 작업 (2026-08-19 — 유료 재추천 인원 수 상한 버튼 비활성화)

- 작업명: 유료 재추천 결제 전 인원 수 선택기의 최대 인원 도달 시 `+` 버튼 비활성화
- 관련 Issue: 확인 필요
- 관련 브랜치: 현재 작업 브랜치
- 진행 상황: 완료 — 선택 가능한 최대 인원에 도달하면 `+` 버튼을 비활성화하고 회귀 테스트 추가

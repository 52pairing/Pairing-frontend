...
- `SIGNING`은 내 서명 완료 여부와 관계없이 `SIGN_PENDING` 단계 전체를 포함
- 계약 상세 PDF 다운로드는 양측 서명이 모두 `SIGNED`일 때만 화면에 노출하며, 서명 화면의 PDF 미리보기 API 사용은 유지
- `SIGNING` 백엔드 배포 및 실제 로그인 세션 기반 응답은 미검증

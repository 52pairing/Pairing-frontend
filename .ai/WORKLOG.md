...
- `/about` 페이지 파일은 저장소에 존재하지 않아 추가로 삭제한 페이지는 없습니다.
- 나머지 `클라이언트 등록`, `프리랜서 등록`, `로그인`, `회원가입` 링크가 유지되고 `회사 소개` 링크가 노출되지 않는 회귀 테스트를 추가했습니다.
- API 변경: 없음.
- 검증: 관련 Jest 1 suite/1 test, 변경 파일 ESLint, 프로덕션 빌드, `git diff --check` 통과.
- 실제 브라우저 확인: 미실행 — 코드·테스트·빌드로 확인.
...
- 서버 등급 조회가 실패한 경우에도 동일한 크기의 비가시 배지가 공간을 유지하며 기존 클라이언트 조회로 폴백하도록 해 닉네임과 메인 문구의 위아래 이동을 방지했습니다.
- 서버 초기값이 있을 때 사용자명·등급이 함께 표시되고 등급 중복 조회가 발생하지 않는 경우와, 초기 등급이 없을 때 사용자명과 배지 자리를 유지한 후 실제 등급을 표시하는 경우를 테스트로 추가했습니다.
- 최초에 첨부 화면을 프리랜서 메인으로 잘못 판단해 적용했던 변경은 모두 되돌렸으며, 실제 대상인 클라이언트 메인에만 반영했습니다.
...
# 2026-08-19 — 유료 재추천 인원 수 상한 버튼 비활성화

- 재추천 결제 전 포지션별 인원 수 선택기에서 선택 가능한 최대 인원에 도달하면 `+` 버튼이 비활성화되도록 수정
- 비활성화 상태에 커서·투명도 스타일과 접근 가능한 버튼 이름 추가
- 최대 인원 도달 시 비활성화 및 상한 미도달 시 정상 증가를 검증하는 Jest 테스트 추가
- 검증: 신규 Jest 테스트 수집·실행 통과, 변경 파일 ESLint, `git diff --check` 통과
- 실제 브라우저·API: 미검증

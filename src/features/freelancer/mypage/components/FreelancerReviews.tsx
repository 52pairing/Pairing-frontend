...
function Tab({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) { return <button type="button" role="tab" aria-selected={active} onClick={onClick} className={`relative min-w-28 px-4 pb-3 text-[11px] font-bold ${active ? "text-brand after:absolute after:inset-x-0 after:-bottom-px after:h-0.5 after:bg-brand" : "text-theme-muted"}`}>{children}</button>; }
function ReceivedItem({ review }: { review: WrittenReview }) { return <article className="py-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-[12px] font-extrabold">{review.reviewerName}</h3><p className="mt-1 text-[10px] font-semibold text-theme-muted">{review.projectTitle}</p></div><div><Stars rating={review.score} /><time className="ml-3 text-[10px] text-theme-muted">{formatDate(review.createdAt)}</time></div></div>{review.content ? <p className="mt-3 text-[11px] font-semibold leading-5 text-theme-secondary">{review.content}</p> : null}</article>; }
...
function Stars({ rating }: { rating: number }) { const rounded = Math.round(Math.max(0, Math.min(5, rating))); return <span className="text-[15px] tracking-1 text-amber-500" aria-label={`${rating}점`}>{"★".repeat(rounded)}<span className="text-theme-muted">{"★".repeat(5 - rounded)}</span></span>; }

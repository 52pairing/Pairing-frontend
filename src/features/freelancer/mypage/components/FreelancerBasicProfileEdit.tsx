...
export function FreelancerBasicProfileEdit() {
  ...
  const save = async () => {
    ...
    try {
      await updateFreelancerProfile({ phone: phone.trim() || undefined, address: address.trim() || undefined, aiMatchingAgreed: profile.aiMatchingAgreed, ...(profileFileId == null ? {} : { profileFileId }) });
  ...
  return (
    <FreelancerMyPageLayout activeMenu="profile">
      <section className="rounded-xl border border-theme bg-surface px-5 py-6 sm:px-7">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><Disabled label="이름" value={profile.name} /><Disabled label="이메일" value={profile.email} /><Disabled label="생년월일" value={profile.birthDate ?? "미등록"} /><Field label="전화번호"><input value={phone} maxLength={13} inputMode="numeric" onChange={(event) => setPhone(formatPhoneNumber(event.target.value))} className={inputClass} /></Field><div className="sm:col-span-2"><Field label="주소"><input value={address} maxLength={255} onChange={(event) => setAddress(event.target.value)} className={inputClass} /></Field></div><Field label="프로필 사진"><input type="file" accept="image/jpeg,image/png" className={inputClass} onChange={async (event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (!file) return; if (!["image/jpeg", "image/png"].includes(file.type) || file.size > 5 * 1024 * 1024) return setMessage("JPG 또는 PNG 파일을 5MB 이하로 선택해 주세요."); try { const uploaded = await uploadFreelancerFile(file, "PROFILE_IMAGE"); setProfileFileId(uploaded.fileId); setMessage("프로필 사진이 업로드되었습니다."); } catch (error) { setMessage(error instanceof Error ? error.message : "사진을 업로드하지 못했습니다."); } }} /></Field>{message ? <p role="status" className="text-[11px] font-semibold text-theme-muted sm:col-span-2">{message}</p> : null}<div className="flex justify-end sm:col-span-2"><button type="button" disabled={saving || reauthRequired} onClick={() => void save()} className="h-11 rounded-md bg-brand px-6 text-[12px] font-bold text-white disabled:opacity-40">{saving ? "저장 중" : "저장"}</button></div></div>
      </section>

    ...
    <main className="min-h-screen bg-[#f5f7fa] px-5 py-5 text-[#172033]">
      <div className="mx-auto w-full max-w-[960px]">
        ...
        <header className="mt-7">
          ...
          <p className="mt-4 text-[11px] font-medium text-[#98a2b3]">카카오 · AI · ML 엔지니어</p>
        </header>
        ...
        <section className="mt-3 rounded-[10px] border border-[#e1e6ed] bg-white px-6 py-5">
        ...
        <section className="mt-3 rounded-[10px] border border-[#e1e6ed] bg-white p-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-[13px] font-bold">계약서 PDF 미리보기</h2>
              <p className="mt-1 text-[11px] text-[#98a2b3]">계약 내용을 확인한 후 서명을 진행해 주세요.</p>
            </div>
            <button type="button" onClick={() => window.print()} className="h-8 cursor-pointer rounded-[7px] border border-[#dce2ea] px-3 text-[11px] font-semibold text-[#667085] transition hover:bg-[#f8fafc]">PDF 다운로드</button>
          </div>

          <PdfPreview />

          <div className="mt-4 rounded-[8px] border border-[#cbdbea] bg-[#eef5fb] px-4 py-3.5">
            <p className="text-[12px] font-bold text-[#344054]">계약 내용을 확인하고 서명해 주세요.</p>
            <p className="mt-2 text-[11px] leading-5 text-[#667085]">서명은 전자 서명으로 처리되며 법적 효력이 있습니다. 계약 내용에 동의하는 경우에만 서명해 주세요.<br />서명 기한: 2026.08.10까지 · 서명하지 않으면 계약이 자동으로 취소될 수 있습니다.</p>
            <button type="button" onClick={() => setIsSignModalOpen(true)} className="mt-3 h-9 cursor-pointer rounded-[7px] bg-[#102846] px-4 text-[11px] font-bold text-white transition hover:bg-[#0c2039]">계약서 서명하기</button>
          </div>
        </section>
      </div>
...
function StatusCard({ title, name, status, active = false, complete = false }: { title: string; name: string; status: string; active?: boolean; complete?: boolean }) {
  const colorClass = complete ? "border-[#a7e8c4] bg-[#effcf4]" : active ? "border-[#c9d9ed] bg-[#eef5fb]" : "border-[#e1e6ed] bg-white";
  const statusClass = complete ? "text-[#16a05d]" : active ? "text-[#2f6fed]" : "text-[#98a2b3]";
  return <article className={`min-h-[116px] rounded-[10px] border p-4 ${colorClass}`}><p className={`text-[10px] font-semibold ${complete ? "text-[#16a05d]" : "text-[#98a2b3]"}`}>{complete ? "✓ " : ""}{title}</p><p className={`mt-3 text-[12px] font-bold ${complete ? "text-[#168653]" : ""}`}>{name}</p><p className={`mt-1.5 text-[10px] font-semibold ${statusClass}`}>{status}</p></article>;
}

function PdfPreview() {
  return (
    <div className="mx-auto mt-5 max-w-[700px] rounded-[9px] border border-[#e2e6ec] bg-[#fafbfc] p-4">
      <div className="mx-auto min-h-[560px] max-w-[570px] bg-white px-10 py-8 shadow-[0_1px_5px_rgba(15,23,42,0.08)]">
        <h3 className="text-center text-[16px] font-bold">프리랜서 용역 계약서</h3>
        <p className="mt-2 text-center text-[9px] text-[#98a2b3]">계약 번호: CNT-2026-00127 · 작성일: 2026.08.02</p>
        ...
        <PdfSection title="4. 기타 조항"><p className="text-[9px] leading-5 text-[#667085]">• 비밀유지: 계약 기간 및 종료 후 관련 정보를 외부에 공개하지 않습니다.<br />• 지식재산권: 계약을 통해 생성된 모든 결과물의 권리는 발주자에게 귀속됩니다.</p></PdfSection>
...
function MiniParty({ title, name, detail }: { title: string; name: string; detail: string }) { return <div className="rounded-[5px] bg-[#f7f8fa] p-3 text-[9px]"><p className="text-[#98a2b3]">{title}</p><p className="mt-3 font-bold">{name}</p><p className="mt-2 text-[#667085]">{detail}</p></div>; }
function MiniRows({ rows }: { rows: string[][] }) { return <div>{rows.map(([label, value]) => <div key={label} className="grid grid-cols-[105px_1fr] border-b border-[#edf0f3] py-2 text-[9px]"><span className="text-[#98a2b3]">{label}</span><span className="font-semibold">{value}</span></div>)}</div>; }
function SignatureBox({ label, name }: { label: string; name: string }) { return <div className="rounded-[6px] border border-[#e2e6ec] py-5 text-center text-[9px]"><p className="text-[#98a2b3]">{label}</p><p className="mt-3 text-[11px] font-bold">{name}</p><div className="mx-auto mt-4 flex h-12 w-12 items-center justify-center rounded-full border border-dashed border-[#d6dbe3] text-[8px] text-[#98a2b3]">전자서명</div></div>; }

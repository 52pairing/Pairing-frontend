...
export function ContractCompleteModal({ contract, onBackToProject, onDownload, backLabel = "프로젝트로 돌아가기" }: ContractCompleteModalProps) {
  ...
  const payLabel = contract.payUnit === "MONTHLY"
    ? `월 ${(contract.payAmount / 10_000).toLocaleString("ko-KR")}만원`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0f172a]/45 px-4">
      <section role="dialog" aria-modal="true" aria-labelledby="contract-complete-title" className="w-full max-w-[520px] rounded-[18px] bg-surface px-10 py-10 shadow-[0_24px_60px_rgba(15,23,42,0.22)]">
        ...
        <h2 id="contract-complete-title" className="mt-7 text-center text-[24px] font-extrabold tracking-[-0.04em] text-theme-primary">{isContractSigned ? "계약서가 체결되었습니다." : "전자 서명이 완료되었습니다."}</h2>
        <p className="mt-4 text-center text-[15px] font-semibold text-theme-secondary">{isContractSigned ? "양측 서명이 완료되었습니다." : "상대방의 서명을 기다리고 있습니다."}</p>

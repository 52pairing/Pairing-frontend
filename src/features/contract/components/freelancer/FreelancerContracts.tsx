...
import type { ContractListItem, ContractListTab } from "@/features/contract/types/contractList";
...
export function FreelancerContracts() {
  ...
  const loadContracts = useCallback(
    () => getContracts({ tab: TAB_CODES[activeStatus], page, size: PAGE_SIZE }),
    [activeStatus, page],
  );
  const { data: contractPage, isLoading, error, reload } = useAsyncData(loadContracts, "계약 목록을 불러오지 못했습니다.");

  const loadTabCounts = useCallback(async (): Promise<Partial<Record<FreelancerContractStatus, { label: string; count: number }>>> => {
    try {
      const rows = await getContractTabCounts();
      const byTab = Object.fromEntries(rows.map((row) => [row.tab, row]));
      return Object.fromEntries(Object.entries(TAB_CODES).map(([label, code]) => [label, byTab[code] ? { label: byTab[code].label, count: byTab[code].count } : undefined]));
    } catch {
      return {};
    }
  }, []);
  const { data: tabRowsData } = useAsyncData(loadTabCounts);
  const tabRows = tabRowsData ?? {};
  ...
  return (
    <main className="min-h-screen bg-surface-subtle pb-8">
      <div className="mx-auto w-full max-w-[1000px] px-4 pt-6 sm:px-5">
        ...
        <p className="mt-2 text-[11px] font-semibold text-theme-secondary">계약 체결 이후 진행 상황을 확인하세요.</p>

        <FreelancerContractStatusTabs activeStatus={activeStatus} onStatusChange={changeStatus} rows={tabRows} />

        {paymentErrorMessage ? <p role="alert" className="mt-4 rounded-lg border border-danger-border bg-danger-surface px-4 py-3 text-[11px] font-semibold text-theme-danger">{paymentErrorMessage}</p> : null}
        ...
        <ListState
          ...
          frameClassName="mt-5 h-32 rounded-xl"
        >

  ...
  return (
    <main className="min-h-[calc(100dvh-60px)] bg-surface-subtle px-5 py-8 text-theme-primary">
      <div className="mx-auto w-full max-w-[520px]">
        ...
        <section className="mt-7 rounded-[14px] border border-theme bg-surface px-7 py-6">
        ...
        <section className="mt-4 rounded-[14px] border border-theme bg-surface px-7 py-6">
          ...
          <div className="mt-5 flex items-center justify-between border-t border-theme pt-5">
            <span className="text-[14px] font-bold">최종 결제 금액</span>
            <strong className="text-[20px] font-extrabold text-brand">{SUCCESS_FEE.toLocaleString("ko-KR")}원</strong>
          </div>
        </section>

        <label className="mt-5 flex cursor-pointer items-center gap-3 text-[12px] font-semibold text-theme-secondary">
          <input type="checkbox" checked={isAgreed} onChange={(event) => setIsAgreed(event.target.checked)} className="h-4 w-4 cursor-pointer rounded border-theme accent-[#17365d]" />
          결제 약관에 동의합니다.
        </label>

        <button type="button" disabled={!isAgreed} onClick={() => setIsPaymentModalOpen(true)} className="mt-5 h-[48px] w-full cursor-pointer rounded-[9px] bg-brand text-[14px] font-bold text-white transition hover:bg-brand disabled:cursor-not-allowed disabled:bg-[#a7b0bf]">
...
function PaymentRow({ label, value, muted = false }: { label: string; value: string; muted?: boolean }) {
  return <div className={`flex items-center justify-between gap-5 ${muted ? "text-theme-muted" : ""}`}><dt className="font-semibold">{label}</dt><dd className="text-right font-bold">{value}</dd></div>;
}

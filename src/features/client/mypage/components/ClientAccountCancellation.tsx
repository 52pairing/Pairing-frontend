...
export function ClientAccountCancellation() {
  ...
  return (
    <ClientMyPageLayout activeMenu="cancel">
      <section className="rounded-xl border border-theme bg-surface px-5 py-6 sm:px-8 sm:py-7">
        ...
        {!canCancel ? <BlockedCancellation blockers={ACCOUNT_BLOCKERS} /> : <CancellationForm agreed={agreed} confirmation={confirmation} canSubmit={canSubmit} onAgreedChange={setAgreed} onConfirmationChange={setConfirmation} onSubmit={() => setOpen(true)} />}
      </section>
      <Modal open={open} onClose={() => setOpen(false)} size="md" labelledBy="cancel-ready-title" describedBy="cancel-ready-description">
        <h2 id="cancel-ready-title" className="text-[20px] font-extrabold tracking-[-0.02em]">회원 탈퇴 API 연동이 필요합니다</h2>
        <p id="cancel-ready-description" className="mt-4 text-[13px] font-semibold leading-6 text-theme-secondary">현재는 디자인과 입력 검증만 구현되어 계정이 실제로 삭제되지 않았습니다. 탈퇴 API가 확정되면 이 단계에서 요청 후 로그아웃 처리합니다.</p>

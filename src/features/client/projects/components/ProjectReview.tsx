    ...
    <ProjectRegisterShell currentStep={STEP} backHref="/client" backLabel="홈으로">
      {phase === "intro" ? (
        <>
          ...
          <div className="mx-auto mt-6 max-w-[720px] rounded-[12px] border border-[#dce2e8] bg-[#fbfcfd] px-5 py-4">
            <p className="text-[11px] font-extrabold text-[#111827]">검수 항목</p>
...
function ReviewHeader({ title, children }: { title: string; children: ReactNode }) {
  return (
    <header className="mx-auto mt-12 max-w-[720px]">
      <h1 className="text-[22px] font-extrabold tracking-[-0.04em] text-[#111827]">{title}</h1>
      <p className="mt-2 whitespace-pre-line text-[12px] font-medium text-[#667085]">{children}</p>
    </header>
  );
}
...
function ResultCard({ item, label }: { item: ProjectPreReviewResponse["items"][number]; label: string }) {
  const summary = (
    <p className="mt-1 text-[11px] text-[#667085]">
      ...
      <span className={`ml-1 font-bold ${item.matchable ? "text-[#12b76a]" : "text-[#f04438]"}`}>
  ...
  if (item.matchable) {
    return <div className="flex items-center justify-between rounded-[10px] border border-[#e1e6ec] bg-white px-5 py-4"><div><p className="text-[13px] font-extrabold text-[#111827]">{label}</p>{summary}</div><StatusIcon matchable /></div>;
  }

  return (
    <div className="rounded-[10px] border border-[#f04438] bg-[#fef4f4] px-5 py-4">
      <div className="flex items-center justify-between"><div><p className="text-[13px] font-extrabold text-[#111827]">{label}</p>{summary}</div><StatusIcon matchable={false} /></div>
...
function BottomBar({ children, align = "between" }: { children: ReactNode; align?: "between" | "end" }) {
  return <div className="mt-8 border-t border-[#e2e7ec] pt-6"><div className={`flex items-center gap-2 ${align === "end" ? "justify-end" : "justify-between"}`}>{children}</div></div>;
}
...
function BackButton({ onClick }: { onClick: () => void }) {
  return <BaseButton onClick={onClick} className="gap-1 border border-[#dce2e8] bg-white text-[#596579] hover:bg-[#f8fafc]"><ChevronLeftIcon size={13} />이전</BaseButton>;
}
function PrimaryButton({ children, onClick, disabled = false }: { children: ReactNode; onClick: () => void; disabled?: boolean }) {
  return <BaseButton onClick={onClick} disabled={disabled} className={disabled ? "cursor-not-allowed bg-[#a7b0bf] text-white" : "bg-[#17365d] text-white hover:bg-[#102a49]"}>{children}</BaseButton>;
}
function SecondaryButton({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return <BaseButton onClick={onClick} className="border border-[#dce2e8] bg-white text-[#596579] hover:bg-[#f8fafc]">{children}</BaseButton>;
}
function DangerButton({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return <BaseButton onClick={onClick} className="border border-[#f04438] bg-white text-[#f04438] hover:bg-[#fef2f2]">{children}</BaseButton>;
}

function CheckItem({ children }: { children: ReactNode }) {
  return <span className="flex items-center gap-2 text-[11px] font-semibold text-[#344054]"><span className="text-[#98a2b3]">✓</span>{children}</span>;
}

function StatusIcon({ matchable }: { matchable: boolean }) {
  return <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full font-bold ${matchable ? "bg-[#e7f7ee] text-[#12b76a]" : "bg-[#fdecec] text-[#f04438]"}`} aria-label={matchable ? "매칭 가능" : "후보 부족"}>{matchable ? "✓" : "!"}</span>;
}

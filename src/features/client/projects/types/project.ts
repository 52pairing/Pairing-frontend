 ...
 *
 * ⚠️ 백엔드 스펙 미확정 상태입니다(.ai/API.md 참고).
 * - 스텝을 거치며 값이 부분적으로만 채워지므로 전부 optional로 둡니다.
 * - 필드/enum 문자열은 화면 기준의 잠정값이며, API 확정 시 조정합니다. (미검증)
 * - API 연동 시에는 이 UI 타입과 별개로 "요청 타입"을 두고
 *   변환 함수(toProjectRegisterRequest)로 매핑하세요. (요청/응답 타입 분리)
 */

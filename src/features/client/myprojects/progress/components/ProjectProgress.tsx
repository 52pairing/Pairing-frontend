    ...
    <section className="mt-6 grid items-start gap-5 lg:grid-cols-[1fr_320px]">
      <div className="flex flex-col gap-3">
        {MEMBERS.map((member, index) => {
          ...
          return (
            <article key={member.id} className="flex min-h-[104px] items-center justify-between rounded-[14px] border border-theme bg-surface px-6 shadow-[0_1px_2px_rgba(15,23,42,0.02)]">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-[14px] font-bold text-theme-primary">{member.name}</h2>
                  ...
                  <p className="mt-2 text-[12px] font-semibold text-theme-secondary">{member.role} · {member.pay}</p>
                </div>
              </div>

              <Link href={`/client/projects/${projectId}/contracts/${member.id}`} className="flex h-9 cursor-pointer items-center gap-1.5 rounded-[8px] border border-[#d8dee7] px-4 text-[12px] font-semibold text-theme-secondary transition hover:bg-surface-subtle">
...
export function ProjectProgressActions({ onComplete, onTerminate }: { onComplete: () => void; onTerminate: () => void }) {
  return (
    <div className="flex items-center gap-2">
      <button type="button" onClick={onTerminate} className="h-9 cursor-pointer rounded-[8px] border border-[#f04438] bg-surface px-4 text-[12px] font-bold text-theme-danger transition hover:bg-danger-surface">중도 종료</button>
      <button type="button" onClick={onComplete} className="h-9 cursor-pointer rounded-[8px] bg-brand px-5 text-[12px] font-bold text-white transition hover:bg-brand">프로젝트 완료 처리</button>
    </div>
  );
}

function InfoRow({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4"><dt className="font-semibold text-theme-muted">{label}</dt><dd className="font-bold text-theme-primary">{value}</dd></div>;
}

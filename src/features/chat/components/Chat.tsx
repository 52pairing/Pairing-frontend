...
export function Chat() {
  ...
  return (
    <div className="h-[calc(100vh-60px)] overflow-hidden bg-[#f5f7fa] px-4 py-4 text-[#111827] sm:px-6 lg:px-8">
      <div className="mx-auto flex h-full max-w-[1120px] flex-col">
        <NegotiationSummary />

        <section className="mt-3 flex min-h-0 flex-1 overflow-hidden rounded-xl border border-[#dfe3e8] bg-white shadow-sm">
...
function NegotiationSummary() {
  return (
    <section className="rounded-xl border border-[#dfe3e8] bg-white px-4 py-3 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-5">
          <span className="text-[11px] text-[#98a2b3]">09:08 · 오늘</span>
      ...
      <p className="mt-2.5 text-[10px] text-[#667085]"><strong className="mr-3 text-[#377cf6]">추가 조건</strong>· 완료 후 1개월 연장 옵션</p>
    </section>
  );
}
...
function SummaryItem({ label, value, last = false }: { label: string; value: string; last?: boolean }) {
  return (
    <div className={`px-4 py-3 ${last ? "" : "border-b border-[#e5e9ef] sm:border-r lg:border-b-0"}`}>
...
function ConversationList({ selectedId, onSelect }: { selectedId: number; onSelect: (id: number) => void }) {
  return (
    <aside className="hidden w-[285px] shrink-0 border-r border-[#dfe3e8] md:block">
      <div className="h-[72px] border-b border-[#dfe3e8] px-5 py-4">
        <h1 className="text-sm font-bold">메시지</h1>
        <p className="mt-1 text-[11px] text-[#98a2b3]">{conversations.length}개의 대화</p>
      </div>
      {conversations.map((conversation) => {
        const isSelected = conversation.id === selectedId;
        return (
          <button key={conversation.id} type="button" onClick={() => onSelect(conversation.id)} className={`flex w-full gap-2.5 border-b border-[#e7eaf0] px-4 py-3.5 text-left hover:bg-[#f5f8fc] ${isSelected ? "bg-[#edf4fa]" : "bg-white"}`}>
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-2 inline-block rounded bg-[#f7f8fa] px-2 py-1 text-[9px] text-[#9aa3b1]">{conversation.role}</span>
            </span>
          </button>
        );
      })}
...
function ChatRoom({ conversation, messages, draft, onDraftChange, onSubmit }: { conversation: Conversation; messages: ChatMessage[]; draft: string; onDraftChange: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col">
      <header className="flex h-[72px] shrink-0 items-center gap-3 border-b border-[#dfe3e8] px-5 sm:px-6">
        <Avatar initials={conversation.initials} active />
        <div>
          <h2 className="text-[13px] font-bold">{conversation.company}</h2>
          <p className="mt-1 text-[10px] text-[#98a2b3]">{conversation.role}</p>
        </div>
      </header>
  ...
  return (
    <div className={`flex items-start gap-3 ${isMine ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`max-w-[80%] ${isMine ? "text-right" : ""}`}>
        <p className={`inline-block rounded-2xl px-3.5 py-2.5 text-left text-xs font-semibold ${isMine ? "rounded-br-md bg-[#102a4c] text-white" : "rounded-bl-md bg-[#f4f5f7] text-[#111827]"}`}>{message.text}</p>

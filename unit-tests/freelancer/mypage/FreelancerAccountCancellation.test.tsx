...
describe("FreelancerAccountCancellation", () => {
  ...
  it("탈퇴할 수 없는 상태면 차단 사유를 나열하고 탈퇴 신청을 막는다", async () => {
    ...
    render(<FreelancerAccountCancellation />);

    expect(await screen.findByText("현재 탈퇴할 수 없는 상태입니다.")).toBeInTheDocument();
    expect(screen.getByText(/서명 대기 계약 1건/)).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "확인하기" })).toHaveAttribute("href", "/contracts");
  ...
  it("미납 수수료로 탈퇴가 거절되면 탈퇴 가능 여부를 다시 조회한다", async () => {
    mockGetEligibility
    ...
    mockWithdrawAccount.mockRejectedValue(
      new ApiException("AC_011", "미납 수수료가 있습니다.", 409),
    );
    const user = userEvent.setup();
